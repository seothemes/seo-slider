...
require_once( 'includes/seo-slider-taxonomy.php' );
...
function seo_slider_scripts_styles() {
	...
	// Slider CSS.
	wp_enqueue_style( 'slider', plugin_dir_url( __FILE__ ) . 'assets/styles/slider.css' );

	// Slick JS.
	wp_enqueue_script( 'slick', plugin_dir_url( __FILE__ ) . 'assets/scripts/slick.min.js', array( 'jquery' ), true );

	// Get slider settings.
	$settings    = get_option( 'seo_slider_settings' );
	$dots        = ( $settings['dots'] ? 'true' : 'false' );
	$arrows      = ( $settings['arrows'] ? 'true' : 'false' );
	$loop        = ( $settings['loop'] ? 'true' : 'false' );
	$autoplay    = ( $settings['autoplay'] ? 'true' : 'false' );
	$speed       = ( $settings['speed'] ? $settings['speed'] : '5000' );
	$transition  = ( $settings['transition'] ? $settings['transition'] : '1000' );
	...
	wp_add_inline_script( 'slick', $slider_settings );

}

add_action( 'admin_init', 'seo_slider_remove_genesis_term_meta', 11 );
/**
 * Remove Genesis Framework term metaboxes.
 */
function seo_slider_remove_genesis_term_meta() {

	remove_action( 'slider_edit_form', 'genesis_taxonomy_archive_options', 10 );
	remove_action( 'slider_edit_form', 'genesis_taxonomy_seo_options', 10 );
	remove_action( 'slider_edit_form', 'genesis_taxonomy_layout_options', 10 );
